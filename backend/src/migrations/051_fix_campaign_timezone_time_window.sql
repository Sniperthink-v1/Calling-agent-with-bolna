-- Migration 051: Fix Campaign Timezone Time Window Check
-- This migration updates the get_next_queued_call function to:
-- 1. Respect campaign_timezone when checking time windows
-- 2. Convert current UTC time to campaign/user timezone before comparison
-- 3. Database stores everything in UTC, conversions happen at query time

-- First, ensure the session timezone is UTC
SET TIME ZONE 'UTC';

-- Note: To permanently set database timezone to UTC, run this manually with your database name:
